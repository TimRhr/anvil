...
|----|-------------|--------|------------|
| **SC-1** | Supply Chain | Der Continuous-Enforcement-Timer führt als root `git reset --hard origin/<branch>` + `bootstrap.sh apply` aus ([systemd/anvil-pull-run](../systemd/anvil-pull-run)). Wer das Repo kontrolliert (Kompromittierung, MITM des Fetch, geleakte Push-Credentials), erhält **root auf allen Servern**. Keine Commit-Signaturprüfung, kein gepinnter Stand. | Signierte Commits/Tags verifizieren (`git verify-commit`/allowed_signers), auf getaggten/gepinnten Stand statt `HEAD` festnageln, **Read-only Deploy-Key**, Branch-Protection, optional manuelle Freigabe vor Apply. → Phase 4 |
